...
export class ExchangeTypeController {
    ...
    getAllExchangeType = async (req: Request, res: Response) => {
        let exchangeTypes: IExchangeType[] = await this.exchangeTypeRepository.find({ loadRelationIds: true });
        return res.json({
            data: exchangeTypes,
    ...
    getExchangeTypeById = async (req: Request, res: Response) => {
        ...
        let ex_type: IExchangeType | null = await this.exchangeTypeRepository.findOne({ where: { id } });
